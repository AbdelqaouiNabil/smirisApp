...
export default function TutorDashboard() {
  ...
  const [editingCourse, setEditingCourse] = useState<CourseOffering | null>(
    null
  );
  ...
  const handleCreateCourse = async (e: React.FormEvent) => {
    ...
    if (
      !["general", "business", "exam_prep", "conversation"].includes(
        newCourse.category
      )
    ) {
    ...
    if (
      ...
      (typeof newCourse.duration_weeks !== "number" ||
        newCourse.duration_weeks < 1 ||
        newCourse.duration_weeks > 52)
    ) {
      setFormError("Die Kursdauer muss zwischen 1 und 52 Wochen liegen.");
      return;
    }
    if (
      newCourse.hours_per_week &&
      (typeof newCourse.hours_per_week !== "number" ||
        newCourse.hours_per_week < 1 ||
        newCourse.hours_per_week > 40)
    ) {
    ...
    if (
      ...
      (typeof newCourse.max_students !== "number" ||
        newCourse.max_students < 1 ||
        newCourse.max_students > 100)
    ) {
      setFormError(
        "Die maximale Teilnehmerzahl muss zwischen 1 und 100 liegen."
      );
      return;
    }
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const bookingsData = (bookingsRes.bookings || []).map((b: any) => ({
        ...
        subject: b.subject || "",
        type: (b.booking_type === "tutor" ? "1-zu-1" : "Kleingruppe") as
          | "1-zu-1"
          | "Kleingruppe",
        status: b.status,
      ...
      setCourseOfferings(
        (coursesResponse.courses || []).map((course) => ({
          ...
          type:
            course.category === "exam_prep"
              ? "Prüfungsvorbereitung"
              : course.max_students === 1
              ? "1-zu-1"
              : "Kleingruppe",
          duration: course.hours_per_week
            ? course.hours_per_week * (course.duration_weeks || 1) * 60
            : 60,
          price: course.price,
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500">
                      {booking.date} um {booking.time} ({booking.duration}min)
                    </div>
                  </div>
                </div>
                        ...
                        <button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              toast({
                                title: "Buchung als abgeschlossen markiert",
                                variant: "default",
                              });
                              loadData();
                            } catch (error: any) {
                              toast({
                                title: "Fehler beim Abschließen der Buchung",
                                description: error.message,
                                variant: "destructive",
                              });
                            }
        ...
        <div className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-800 p-4 rounded mb-4 flex items-center">
          ...
          <div>
            <div className="font-semibold">
              Ihr Profil ist noch nicht verifiziert.
            </div>
            <div>
              Sie können erst Kurse erstellen und veröffentlichen, wenn Ihr
              Profil von einem Administrator überprüft und freigeschaltet wurde.
            </div>
          </div>
        </div>
      )}
            ...
            <div className="absolute right-0 mt-2 w-96 bg-white border border-gray-200 rounded-xl shadow-lg z-50 max-h-96 overflow-y-auto">
              ...
              {notifications.length === 0 ? (
                ...
                notifications.map((n) => (
                  <div
                    key={n.id}
                    className={`p-4 border-b last:border-b-0 hover:bg-emerald-50 transition ${n.is_read ? 'opacity-60' : ''}`}
                  >
          ...
          <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-lg relative">
            ...
            <form onSubmit={handleCreateCourse} className="space-y-4">
              ...
              <button
                ...
                className="w-full bg-emerald-600 text-white py-2 rounded hover:bg-emerald-700 transition flex items-center justify-center"
              >
                {isSubmitting && (
                  <span
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Willkommen zurück! Verwalten Sie Ihre Kurse und Buchungen.
          </p>
        </div>
        ...
        <div>
          ...
          {activeTab === "earnings" && (
            <div className="bg-white rounded-xl shadow-md p-8 text-center">
              ...
              <p className="text-gray-600">
                Detaillierte Einnahmenübersicht wird in der nächsten Version
                verfügbar sein.
              </p>
            </div>
          )}
          {activeTab === "settings" && (
            <div className="bg-white rounded-xl shadow-md p-8 text-center">
              ...
              <p className="text-gray-600">
                Profil- und Kontoeinstellungen werden in der nächsten Version
                verfügbar sein.
              </p>
            </div>
          )}
